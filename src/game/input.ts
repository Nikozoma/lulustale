...
const TOUCH_RADIUS = 76;

export function createInputController(canvas: HTMLCanvasElement, viewport: Size) {
  ...
  canvas.addEventListener("pointerdown", (event) => {
    const point = pointerToVirtual(canvas, viewport, event);
  ...
  canvas.addEventListener("pointermove", (event) => {
    ...
    movementPointer.current = pointerToVirtual(canvas, viewport, event);
    event.preventDefault();
  });
...
function pointerToVirtual(canvas: HTMLCanvasElement, viewport: Size, event: PointerEvent): WorldPoint {
